...
!
!     Take-off angle search mode routines for RAYINVR
!
!     ----------------------------------------------------------------
module AutoTrace
...
!
      if(idump.eq.1) write(12,5) ifam,ir,npt,xr(npt),zr(npt),   &
       ...
       layer,iblk,id,iwave
5     format(i4,i3,i4,2f8.3,2f8.2,2f7.2,4i3)
...
!
      if(l.lt.layer1.or.l.gt.nlayer.or.(l.ge.nlayer.and.idr.gt.1)) then
        iflag=1
        return
      end if
!
...
!
!     calculate the velocity at the shot point and the
!     maximum velocity between the shot and the top and
!     bottom of the layer for which rays are being searched
!     for directly beneath the shot point
!
      include 'rayinvr.par'
      include 'rayinvr.com'
!
