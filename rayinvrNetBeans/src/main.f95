...
!
!        31 -- output: velocity model sampled on a uniform grid
!
...
!
      ...
      data nray/prayf*-1/,itt/ptrayf*0/,modi/player*0/,         &
          ...
          insmth/pncntr*0/
!
      character(LEN=*), parameter :: inputR = 'r.in'
      character(LEN=*), parameter :: inputV = 'v.in'
      ...
      i33=0
!
      open(unit=10, file=inputR, status='old', iostat = IO)
...
1       format(' '/' '/' ')
        ...
        open(unit=20, file=inputV, status='old')
        iunit=20
      end if
!
...
70       continue
!
         if(isep.eq.2 .and.((itx.gt.0 .and. ntt.gt.1) .or.              &
              idata.ne.0 .or. itxout.gt.0))                             &
            call plttx(ifam,itt,iszero,idata,iaxlab, xshota,idr,        &
              nshot,itxout,ibrka,ivraya,ttunc,itrev,xshotr,             &
...
!
      if(abs(modout).ne.0) &
            call modwr(modout,dxmod,dzmod,modi,ifrbnd,frz,xmmin,xmmax)
!
