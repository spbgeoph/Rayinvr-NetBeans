...
!    +       xtmin,xtmax,ntickx,ndecix,'DISTANCE (m)',12,albht)
!       call axis(orig,sep,zmin,zmax,zmm,zscale,90.,1,
!    +       ztmin,ztmax,ntickz,ndeciz,'DEPTH (m)',9,albht)
        call axis(orig,sep+zmm,xmin,xmax,xmm,xscale,0.,-1,          &
...
!    +           xtmint,xtmaxt,ntckxt,ndecxt,'OFFSET (m)',10,albht)
               call axis(orig,orig,xmint,xmaxt,xmmt,xscalt,0.,1,            &
...
!   ufispb
!   Error: 'b' argument of 'sign' intrinsic  must be the same type and kind as 'a'
!   So expression for 'fcalc' has been slightly changed
                      !fcalc=sign(1.,icalc(j))
                      fcalc=sign(1.,real(icalc(j)))
...
!                     write(17,5) xcalc(j),twrite,ttunc,abs(icalc(j))
