...
55        format('***  ray stopped - reflected from pinchout  ***')
          vr(n,2)=0.
          iflag=1
          return
        end if
...
35      format('***  ray stopped - converted ray cannot reflect/refract  ***')
        vr(n,2)=0.
        iflag=1
        return
      end if
...
!     if(ir.gt.0.and.modout.eq.1.and.layer.eq.idray(1)-1.
!    +  and.idray(2).eq.1) write(32,155) xr(n),zr(n),4
      return
!
!     ray intersects lower boundary
!
...
!     write(0,*) a1,crita,diff,crit
!     added abs to above statement and if-then-else below is new Jul 8/98
        if(fid.ne.fid1) then
          fid=fid1
          id=-id
        end if
...
!
!     if(ir.gt.0) write(45,*) xr(n),57.29577951*a1
      return
!
